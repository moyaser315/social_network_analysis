...
std::string IDs[] = {"users", "user", "topics",    "topic",    "posts", "post",
                     "name",  "id",   "followers", "follower", "body"};
int freq[11] = {0};
int errors;

/// @brief strips the line from every leading and trailing non letters
/// charachters
/// @param line
...
/// @param unmini // the file as an xml without any stripped spaces will be
/// passed by ref
/// @param sp // the string with spaces in values
/// @return  // the string without any spaces at all to take less computtion
/// power while building tree time complexity :
std::string toline(std::string fpath, std::string &unmini, std::string &sp) {
    // open file
    std::ifstream file;
    file.open(fpath);

    std::string res = "", line, nonspace = "";
...
/// @param st the position to start parsing from passed by refrence to continue
/// parsing after the tag
/// @param f passed as refrence to specifify if it's closing or opening tag 1 :
/// closing ,0  : opening
...
std::string get_tag(std::string line, int &st, bool &f) {
    ...
    // so we will substring from index 2 </x-->2 and end at len of word
    // excluding / if it's opening tag we start from index 1 and end at len of
    // word
...
/// @param f know if it's opening or closing ,opening = +1 ,closing = -1,the
/// best case is that all tags are valued 0
